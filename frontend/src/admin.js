...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderRapportJournalierOfficiel() {
        ...
        // ══════════════════════════════════════════════════════════════
        // ÉTAPE 1 : CHOIX DIRECT ENTRE LES DEUX ÉCOLES
        // ══════════════════════════════════════════════════════════════
        if (!selectedEco) {
            ui.content.innerHTML = `
                <!-- En-tête Prestigieux du Choix d'Établissement -->
                <div class="mb-10 text-center max-w-3xl mx-auto">
                    ...
                    <p class="text-xs text-amber-300/90 font-bold uppercase tracking-widest mt-1">
                        Gouvernance Institutionnelle • Direction Générale
                    </p>
                    <p class="text-xs text-gray-400 mt-2 max-w-xl mx-auto leading-relaxed">
                        Sélectionnez l'établissement dont vous souhaitez consulter les détails et le rapport d'activités certifié :
                    </p>
                </div>

                <!-- SECTION SUPER-ADMIN : RAPPORTS EN ATTENTE DE VISA (SI APPLICABLE) -->
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2.5">
                                ...
                                <h3 class="text-sm font-black text-amber-300 uppercase tracking-wider">
                                    📥 Rapports Journaliers Soumis — En Attente de Votre Visa d'Approbation (${pendingReports.length})
                                </h3>
                            </div>
                            <span class="text-xs text-gray-400 font-bold">Action requise pour transmission à la Direction Générale</span>
                        </div>
                                ...
                                <div class="p-5 rounded-2xl bg-black/40 border border-amber-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    ...
                                    <div class="flex items-center gap-2 shrink-0">
                                        <button type="button" onclick="approuverRapportParSuperAdmin('${rep.id}')"
                                            ...
                                            <i data-lucide="check-check" class="w-4 h-4"></i> Viser & Approuver pour la Direction Générale
                                        </button>
                ...
                <!-- LES 2 GRANDES CARTES VIP DE CHOIX D'ÉTABLISSEMENT -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto mb-10">
                    
                    <!-- CARTE 1 : C.S. HARMONIE (PRIMAIRE) -->
                    ...
                    <!-- CARTE 2 : G.S. RETROUVAILLES (SECONDAIRE) -->
        ...
        // ══════════════════════════════════════════════════════════════
        // ÉTAPE 2 : DÉTAILS DU RAPPORT OFFICIEL DE L'ÉCOLE CHOISIE
        // ══════════════════════════════════════════════════════════════
        ...
        ui.content.innerHTML = `
            <!-- Barre Supérieure avec Retour au Choix des Écoles -->
            <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-4">
                    <button type="button" onclick="resetEcoleRapportChoice()"
                        ...
                        <span>←</span>
                        <span>Choisir une Autre École</span>
                    </button>
                    <div>
                        <div class="flex items-center gap-2">
                        ...
                        <p class="text-xs text-amber-300/90 font-bold uppercase tracking-widest mt-0.5">Gouvernance Institutionnelle • Direction Générale</p>
                    </div>
                </div>
            ...
            ${isSuperAdminRole && pendingReports.length > 0 ? `
                <div class="glass-panel p-6 rounded-3xl border-2 border-amber-500/40 bg-gradient-to-r from-amber-500/10 via-yellow-500/5 to-transparent mb-8 shadow-2xl">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-xs text-gray-400">La Direction Générale ne verra ces rapports qu'après votre approbation</span>
                    </div>
                        ...
                        ${pendingReports.map(rep => `
                            <div class="p-5 rounded-2xl bg-black/40 border border-amber-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <button type="button" onclick="approuverRapportParSuperAdmin('${rep.id}')"
                                        class="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-gray-950 font-black text-xs uppercase tracking-wider rounded-xl shadow-lg transition flex items-center gap-1.5 cursor-pointer">
                                        <i data-lucide="check-check" class="w-4 h-4"></i> Viser & Transmettre à la Direction Générale
                                    </button>
                                    <button type="button" onclick="demanderRevisionRapport('${rep.id}')"
                                        class="px-3 py-2.5 bg-white/10 hover:bg-white/20 text-gray-300 text-xs font-bold rounded-xl transition cursor-pointer">
                                        🔄 Demander Correction
